...
function is_hometest()
{
    for option in "$@"
    do
        if [ "$option" = "-hometest" ]; then
            return 0
        fi
    done
...
# If hometest and steam faulted, restart
if is_hometest "$@" && [ $STATUS -eq $SEGV_EXITCODE ] ; then
	echo "Restarting Steam due to SIGSEGV..."
	RESTART=1
fi
